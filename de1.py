...
def render_pdf_pages(pdf_path, dpi=120):
...
def compare_pdf_images(pdf1, pdf2, dpi=120, output_dir="diff_images"):
    ...
    for i, (p1, p2) in enumerate(zip(pages1, pages2)):
        ...
        # 🔹 Show PDF1 & PDF2 only (side-by-side, no diff image)
        html = f"""
        <h3>Page {i+1}</h3>
        <p>{diff_status}</p>
        <div style="display:flex;gap:30px;align-items:flex-start;">
            <div>
                <p><b>{os.path.basename(pdf1)}</b></p>
                <img src="{p1_path}" width="450">
            </div>
            <div>
                <p><b>{os.path.basename(pdf2)}</b></p>
                <img src="{p2_path}" width="450">
            </div>
        </div>
        <hr>
        """
